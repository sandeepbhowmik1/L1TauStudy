...
for ev in range (0, nentries):
    ...
    for i in range(0, treeIn.tauPt.size()):
        if abs(treeIn.tauEta[i]) < etaMin:
            continue
        if abs(treeIn.tauEta[i]) > etaMax:
            continue
        if saveOnlyOS and not treeIn.isOS:
            continue
        bkgSubW[0] = 1. if treeIn.isOS else -1.
        Nvtx[0] = treeIn.Nvtx
        tauPt[0] = treeIn.tauPt[i]
        tauEta[0] = treeIn.tauEta[i]
        tauPhi[0] = treeIn.tauPhi[i]
        l1tPt[0] = -1
        l1tEta[0] = -5
        l1tPhi[0] = -5
        for j in range (0, len(workingPoints)):
            l1tPtIso[j][0] = 0
            l1tPtNonIso[j][0] = 0
            for k in range(0, treeIn.l1tPt.size()):
                DeltaR = math.sqrt((treeIn.tauEta[i]-treeIn.l1tEta[k])**2 + (treeIn.tauPhi[i]-treeIn.l1tPhi[k])**2)
                if DeltaR < 0.5:
                    l1tPt[0] = treeIn.l1tPt[k]
                    l1tEta[0] = treeIn.l1tEta[k]
                    l1tPhi[0] = treeIn.l1tPhi[k]
                    if treeIn.l1tPt[k] > workingPoints[j]:
                        l1tPtNonIso[j][0] = 1
                        if treeIn.l1tIso[k]:
                            l1tPtIso[j][0] = 1
        treeOut.Fill()
